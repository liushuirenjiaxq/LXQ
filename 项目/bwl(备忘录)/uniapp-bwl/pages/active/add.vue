	...
	<view class="content">
		<view class="header">
			<cu-custom bgColor="bg-gradual-blue" :isBack="true">
				...
				<block slot="content">{{ edit.id==0?"新建备忘":"编辑备忘"}}</block>
			</cu-custom>
		</view>
		<view class="text-area">
			...
			<view class="textarea_from">
				<u--form :model="loginForm" ref="uLoginForm">
					<view>
						<text class="title">{{ edit.id==0?"今日备忘":"备忘信息"}}</text>
						<br />
					...
					<u-form-item prop="text">
						<u--textarea v-model="loginForm.text" @focus="passwordF_B" @blur="passwordF_B" height=160
							count :maxlength="maxlength" placeholder="请输入备忘内容" border="bottom" confirmType="done"></u--textarea>
					</u-form-item>
					<view class="btn_Box">
						<button type="primary" class="submitBtn" @click="add()">{{ edit.id==0?"备忘新建":"修改备忘"}}</button>
					...
					<view class="rect" @tap.stop>
						<u-form>
							<view class="title" style="text-align: center;margin-top: 10px;">
								<text style="font-size: 15px;">新增成功，是否现在去查看？</text>
							</view>
...
<script>
	export default {
		...
		onLoad(opts) {
			//opts.id不是数字类型;转化为数字之后再赋值
			if(opts.id == 0){
				this.edit.id = opts.id - '0';
			}else{
				this.edit.id = opts.id - '0';
				this.loginForm.text = decodeURIComponent(opts.text);
				this.loginForm.title = decodeURIComponent(opts.title);
				this.edit.text = decodeURIComponent(opts.text);
				this.edit.title = decodeURIComponent(opts.title);
			}
		},
		computed: {
			//取用户的ID,判断用户是否登录
		...
		methods: {
			...
			//修改备忘录
			Bedit(){
				if(this.loginForm.text == this.edit.text && this.loginForm.title == this.edit.title){
					uni.showToast({
						title: '内容未改变',
						icon: 'none'
					});
				...
				uni.request({
					url: this.$api + '/content/update',
					data: {
						text: this.loginForm.text,
						title: this.loginForm.title,
						time: this.dateFormat(this.date),
						id: this.edit.id
					},
					method: 'post',
					header: {
					...
					success: (res) => {
						if(res.data.code === 200){
							this.loginForm.time = this.dateFormat(this.date)
							let obj = new Object()
							Object.assign(obj, this.loginForm)
							obj.id  = this.edit.id;
							this.$store.dispatch('updateDiary',obj)
							uni.showToast({
								title: '修改成功',
								icon: 'success'
							});
			...
			//添加备忘录
			add() {
				if(this.loginForm.text == ""||this.loginForm.title == ""){
					uni.showToast({
						title: '内容为空',
						icon: 'none'
					});
					...
					uni.request({
						...
						success: (res) => {
							if(res.data.code === 200){
								...
								this.loginForm.time = this.dateFormat(this.date)
								let temp = this.loginForm;
								temp.id = res.data.data;
								this.$store.commit('addDiary',temp)
								//弹出层控制
								this.show = true
							}else{
								console.log(err)
